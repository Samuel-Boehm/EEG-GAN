...
def multi_class_time_domain(X_real:np.ndarray, X_fake:np.ndarray, y_real:np.ndarray,
                            y_fake:np.ndarray, class_mapping:dict, channels:list) -> dict:
    '''
    ...
    for classLabel in class_mapping.values():
        ...
        for i in range(n_channels):
            plot_time_domain(real_dict[classLabel][:, i, :], ax=axs[i], title=channels[i], show_std=True, label='real')
            plot_time_domain(fake_dict[classLabel][:, i, :], ax=axs[i], title=channels[i], show_std=True, label='fake')
