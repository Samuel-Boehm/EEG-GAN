...
class ConvBlock(nn.Module):
    ...
    def __init__(self, n_filters, stage, kernel_size, is_generator=False, **kwargs):
        ...
        groups = int(
            n_filters / ((stage) * 2)
        )  # for n_filters = 120: 60, 30, 20, 15, 12, 10
        ...
        self.convolutions.append(
            WS(nn.Conv1d(n_filters, n_filters, kernel_size=1, stride=stride, padding=0))
        )
        self.convolutions.append(nn.LeakyReLU(0.2))
    ...
    def forward(self, x, **kwargs):
        ...
        for i, conv in enumerate(self.convs):
            ...
            if self.split_in_channels:
                tmp_input = tmp_input[
                    :,
                    int(i * self.in_channels_per_conv) : int(
                        i * self.in_channels_per_conv + self.in_channels_per_conv
                    ),
                ]

            if self.reflective:
                tmp_input = nn.functional.pad(
                    tmp_input,
...
def create_multiconv_for_stage(n_filters: int, i_stage: int):
    ...
    return MultiConv1d(
        conv_configs, n_filters, n_filters, split_in_channels=True, reflective=True
    )
