...
class CriticBlock(nn.Module):
    ...
    def __init__(
        self, intermediate_sequence: nn.Sequential, in_sequence: nn.Sequential
    ) -> None:
        super().__init__()
...
class Critic(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, **kwargs):
        embedding: torch.Tensor = self.label_embedding(y.to(torch.long)).view(
            y.shape[0], 1, self.n_samples
        )
