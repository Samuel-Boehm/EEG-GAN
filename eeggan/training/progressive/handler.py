...
class SpectralProgessionHandler(ProgressionHandler):
    def __init__(self, discriminator: ProgressiveDiscriminator, generator: ProgressiveGenerator, 
        spectral_discriminator: ProgressiveDiscriminator, n_stages: int, use_fade: bool,
        epochs_fade: int = None, current_stage: int = 0,
        current_alpha: float = 1, freeze_stages=False):
        
        super().__init__(discriminator, generator, n_stages, use_fade, epochs_fade, current_stage, current_alpha, freeze_stages)
    ...
    def set_progression(self, current_stage: int, current_alpha: float):
        ...
        self.discriminator.cur_block = self.n_stages - current_stage - 1
        self.discriminator.alpha = current_alpha
        self.spectral_discriminator.cur_block = self.n_stages - current_stage - 1
        self.spectral_discriminator.alpha = current_alpha
                

    def get_trainable_discriminator_parameters(self):
        if not self.freeze_stages:
            return self.discriminator.parameters(), self.spectral_discriminator.parameters()
        else:
            return (self.discriminator.blocks[self.n_stages - self.current_stage - 1].parameters(), 
                    self.spectral_discriminator.blocks[self.n_stages - self.current_stage - 1].parameters())