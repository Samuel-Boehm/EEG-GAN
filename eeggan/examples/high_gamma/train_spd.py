...
def train(dataset_name: str, dataset_path: str, deep4s_path: str, result_path: str,
          progression_handler: ProgressionHandler, trainer: SpectralTrainer, n_batch: int, lr_d: float, lr_g: float,
    ...
    for stage in range(progression_handler.current_stage, progression_handler.n_stages):
        ...
        # Set spectral discriminator
        Dsp = SpectralDiscriminator(n_samples=X_block.shape[2], n_channels=21, n_classes=2, spectral='nonlinear')
        Dsp.apply(weight_filler)
        Dsp = to_cuda(Dsp)
        Osp = optim.Adam(Dsp.parameters(), lr=.005, betas=(0., 0.99))

        trainer.sp_discriminator = Dsp
        trainer.sp_optim = Osp
        ...
        # discriminator plotter:
        discriminator_plot = DiscriminatorSpectrum(pyplot.figure(), plot_path, f'disc_freqs_stage_{stage}_')
        
        trainer.add_event_handler(event_name, discriminator_plot)
        ...
        trainer.remove_event_handler(discriminator_plot, event_name)  # spectral_handler.remove() does not work :(
